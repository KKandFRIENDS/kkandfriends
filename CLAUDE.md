...
## 봇 색채 제거 (2026-09-09 KK 지적 → 스킬 v3.3 §5)

KK 지적: "발작"이라는 표현이 너무 잦고, 글이 사람이 쓴 것 같지 않다.
아카이브 34편 전수 측정 결과 원인은 단어 하나가 아니라 **구문 습관**이었다.

- `A가 아니라 B다` **153회 (편당 4.5회, 한 편 최대 10회)**
- `진짜 ~는` 36회 · `Bottom line` 34편 중 25편

**규칙은 스킬 v3.3 §5가 원본이다. 여기엔 한도만 적는다** (계정 스킬 업로드 전까지의 안전망):

- `아니라` + `아니다` 합계 **글당 2회 이하.** 두 번째부터는 긍정형 단언으로,
  또는 대비를 문장 두 개로 쪼갠다 ("모델은 답을 만들었다. 보내지 않았을 뿐이다.")
- **시그니처는 다섯 중 둘만** — `Bottom line:` / `💡` / 팩트-해석 분리 / Skin in the game /
  두 줄 대비. 매 글에 다 넣지 말 것. 조합은 매번 바꾼다.
- **`발작` 계열 병리 은유**(발작·경련·마비·출혈·심정지) 한 달에 한 번 이하. `진짜 ~는` 1회 이하.
- 마무리 장치 6종으로 확장 — **"그냥 멈추기"도 정답이다.**
- 짧은 단문만 늘어놓지 말 것. 단락마다 긴 문장 1개 이상, 단문 3연타는 2회까지.
- 사람의 흔적 2개 — 곁가지 / 모르는 것 인정 / 자기 정정 / 질감 디테일 / 대칭 깨기

> 문체가 약해서 기계처럼 읽히는 게 아니다. 같은 부품이 같은 자리에 매번 박혀서 그렇다.
