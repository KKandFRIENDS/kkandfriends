-- Chief-authored DAILY DESK / KK WEEKLY drafts enter the same approval flow
-- as automated drafts. The public site never calls this function directly;
-- only the VPS editorial bridge may execute it.
...
revoke all on function public.editorial_manual_create(date,jsonb,text) from public,anon,authenticated;
grant execute on function public.editorial_manual_create(date,jsonb,text) to service_role;
