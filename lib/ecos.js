...
export async function fetchEcosKeyStats() {
  ...
  // to open the probe. A 0/15 here means the PICKS regexes miss the BOK's
  // current labels; no line at all means this code never ran.
  console.log(`ecos: ${picks.length}/${PICKS.length} indicators matched (${res.rows.length} rows returned)`);
  return picks;
}
