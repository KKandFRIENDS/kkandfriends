...
## 사이트 디자인 기준 (2026-09-25 KK 결정)

- **브랜드 색은 블루 하나.** `#4A90D9` / 밝은 톤 `#7AB8F5` / 어두운 톤 `#4A7AAA`.
  새 글 템플릿에 골드(`#E8A800`, `#C8A24E`)를 쓰지 말 것. (주황은 "디지털 자산" 분류 태그에만 남아 있다.)
  블루 바탕 버튼의 글씨는 흰색이 아니라 진한 색 `#060810` (흰 글씨는 대비 미달).
- **폰트:** 본문·버튼·UI는 Pretendard(`var(--font-sans)`), 제목만 Playfair + 한글은 Noto Serif KR
  (`var(--font-serif)`). Source Serif 4는 퇴역. 홈은 Noto Serif KR을 받지 않는다(한글 제목은 Pretendard).
- **글씨 크기 하한 11px.** 모바일 탭 영역은 44px 기준.
- **한국어 위주.** 메뉴·버튼·라벨에 "English / 한국어" 병기 금지. 브랜드명(THOUGHTS, KK, Daily Desk)만 영어.
- **경력 표기:** JP Morgan · Bank of America를 앞세운다. ANZ는 경력 띠에서 빼고, 타임라인에서는 작은 보조 줄로만.
