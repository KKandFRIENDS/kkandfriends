...
- If the leading candidate still fails after one editor-guided rewrite, the editor tries the next two eligible ranked candidates with fresh research, writing and review. When the stored ranking has no eligible alternative, it performs one fresh ranking round that excludes rejected topics before declaring final failure.
- A malformed JSON response from any model stage is retried once with the same bounded schema and evidence rules.
- Thursday AI scanning includes the official Google DeepMind, Microsoft Research and OpenAI news feeds. Saturday Korea scanning adds the FSC press-release list and Korea-localized Google News discovery signals; discovery signals may select a topic but cannot support a factual claim.
- At 20:00 KST on Saturday, the VPS checks the number of editions actually published since Monday. If fewer than three are published, it sends one Telegram warning before Sunday's weekly run; it never publishes a draft automatically.
