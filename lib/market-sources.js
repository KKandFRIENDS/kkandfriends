...
// ─── Korea close (the 17:30 KST brief) ──────────────────────────────────────
// Domestic indices/FX plus the overnight offshore levels a Korean desk checks
// against. Yahoo has no reliable free feed for investor-flow (외국인/기관 수급),
// so the brief must not invent it — the prompt tells the model to say so.
const KR_SYMBOLS = [
  { sym: '^KS11',    name: '코스피',      kind: 'pct',  digits: 2 },
  { sym: '^KQ11',    name: '코스닥',      kind: 'pct',  digits: 2 },
  { sym: 'KRW=X',    name: '원/달러',     kind: 'pct',  digits: 1 },
  { sym: '005930.KS', name: '삼성전자',   kind: 'pct',  digits: 0 },
  { sym: '000660.KS', name: 'SK하이닉스', kind: 'pct',  digits: 0 },
  { sym: '^N225',    name: '닛케이',      kind: 'pct',  digits: 2 },
  { sym: '000001.SS', name: '상하이종합', kind: 'pct',  digits: 2 },
  { sym: '^GSPC',    name: 'S&P 500',    kind: 'pct',  digits: 2 },
  { sym: '^TNX',     name: '미 10년물',   kind: 'rate', digits: 3 },
  { sym: 'DX-Y.NYB', name: '달러인덱스',   kind: 'pct',  digits: 2 },
];
...
const KR_FEEDS = [
  googleNews('코스피 when:1d'),
  googleNews('(코스닥 OR 원달러 환율 OR 외국인 순매도) when:1d'),
  googleNews('(한국은행 OR 국고채 OR 반도체 삼성전자 SK하이닉스) when:1d'),
];

// Korean-market quotes for the close-of-session brief.
export async function fetchKoreaQuotes() {
  const results = await Promise.all(KR_SYMBOLS.map((s) => fetchQuote(s).catch(() => null)));
  return results.filter(Boolean);
}

// Korean-language headlines for the close-of-session brief.
export async function fetchKoreaHeadlines(limit = 20) {
  return fetchFeeds(KR_FEEDS, limit);
}
...
async function fetchQuote(s) {
  ...
  // `meta.regularMarketPrice` is the authoritative latest price; the daily
  // `closes` array sometimes LAGS it by a session (the newest row arrives late).
  // Taking closes[-1] blindly therefore published a day-old level with a change
  // computed against the wrong pair — observed live 2026-07-25 on ^KS11, where
  // the same symbol read +4.40% and then -5.72% fifteen minutes apart.
  // So: trust meta for `last`, then decide whether the array's final row is that
  // same session (→ previous close is the row before it) or an older one
  // (→ that row IS the previous close).
  const lastRow = closes.length ? closes[closes.length - 1] : undefined;
  let last;
  let prev;
  if (Number.isFinite(meta.regularMarketPrice)) {
    last = meta.regularMarketPrice;
    const rowIsSameSession =
      Number.isFinite(lastRow) && Math.abs(lastRow - last) <= Math.abs(last) * 1e-6;
    prev = rowIsSameSession ? closes[closes.length - 2] : lastRow;
  } else {
    last = lastRow;
    prev = closes.length > 1 ? closes[closes.length - 2] : undefined;
  }
  if (!Number.isFinite(prev)) prev = meta.previousClose ?? meta.chartPreviousClose;
...
export async function fetchHeadlines(limit = 24) {
  return fetchFeeds(FEEDS, limit);
}

// Round-robins across the given feeds, de-duplicating, up to `limit` titles.
